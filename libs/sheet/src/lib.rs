...
impl ExtractStyle for StyleSheetProperty {
    fn extract(&self) -> String {
        format!("{}{{{}:{}}}", merge_selector(&self.class_name, self.selector.as_ref()), self.property, convert_theme_variable_value(&self.value))
    }
}

static VAR_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"\$\w+").unwrap());
static INTERFACE_KEY_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"^[a-zA-Z_$][a-zA-Z0-9_$]*$").unwrap());

fn convert_interface_key(key: &str) -> String {
    if INTERFACE_KEY_RE.is_match(key) { key.to_string() } else { format!("[`{}`]", key.replace("`", "\\`")) }
}
...
fn deserialize_btree_map_u8<'de, D>(deserializer: D) -> Result<BTreeMap<String, PropertyMap>, D::Error>
where
    D: Deserializer<'de>,
{
    let mut result: BTreeMap<String, PropertyMap> = BTreeMap::new();
    for (key, value) in BTreeMap::<String, BTreeMap<String, BTreeMap<String, HashSet<StyleSheetProperty>>>>::deserialize(deserializer)? {
...
impl StyleSheet {
    ...
    pub fn add_property(&mut self, class_name: &str, property: &str, level: u8, value: &str, selector: Option<&StyleSelector>, style_order: Option<u8>, filename: Option<&str>) -> bool {
        ...
        self.properties.entry(filename.unwrap_or_default().to_string()).or_default().entry(style_order.unwrap_or(255)).or_default().entry(level).or_default().insert(StyleSheetProperty { class_name: class_name.to_string(), property: property.to_string(), value: value.to_string(), selector: selector.cloned() })
    }

    pub fn add_import(&mut self, file: &str, import: &str) {
        self.global_css_files.insert(file.to_string());
        self.imports.entry(file.to_string()).or_default().insert(import.to_string());
    }

    pub fn add_font_face(&mut self, file: &str, properties: &BTreeMap<String, String>) {
        self.global_css_files.insert(file.to_string());
        self.font_faces.entry(file.to_string()).or_default().insert(properties.clone());
    }

    pub fn add_css(&mut self, file: &str, css: &str) -> bool {
        self.global_css_files.insert(file.to_string());
        self.css.entry(file.to_string()).or_default().insert(StyleSheetCss { css: css.to_string() })
    }

    pub fn add_keyframes(&mut self, name: &str, keyframes: BTreeMap<String, Vec<(String, String)>>, filename: Option<&str>) -> bool {
        let map = self.keyframes.entry(filename.unwrap_or_default().to_string()).or_default().entry(name.to_string()).or_default();
        if map == &keyframes {
            return false;
        }
        map.clear();
    ...
    pub fn rm_global_css(&mut self, file: &str, single_css: bool) -> bool {
        ...
        for map in self.properties.entry(property_key.clone()).or_default().values_mut() {
            for props in map.values_mut() {
                props.retain(|prop| if let Some(StyleSelector::Global(_, f)) = prop.selector.as_ref() { f != file } else { true });
            }
            // remove empty map
        ...
        if self.properties.get(&property_key).and_then(|v| if v.is_empty() { None } else { Some(()) }).is_none() {
    ...
    pub fn update_styles(&mut self, styles: &HashSet<ExtractStyleValue>, filename: &str, single_css: bool) -> (bool, bool) {
        let mut collected = false;
        let mut updated_base_style = false;
        for style in styles.iter() {
            match style {
                ExtractStyleValue::Static(st) => {
                    if let Some(StyleProperty::ClassName(cls)) = style.extract(if !single_css { Some(filename) } else { None })
                        && self.add_property(&cls, st.property(), st.level(), st.value(), st.selector(), st.style_order(), if !single_css { Some(filename) } else { None })
                    {
                        collected = true;
                ...
                ExtractStyleValue::Dynamic(dy) => {
                    if let Some(StyleProperty::Variable { class_name, variable_name, .. }) = style.extract(if !single_css { Some(filename) } else { None })
                        && self.add_property(&class_name, dy.property(), dy.level(), &format!("var({})", variable_name), dy.selector(), dy.style_order(), if !single_css { Some(filename) } else { None })
                    {
                        collected = true;
                ...
                ExtractStyleValue::Keyframes(keyframes) => {
                    if self.add_keyframes(&keyframes.extract(if !single_css { Some(filename) } else { None }).to_string(), keyframes.keyframes.iter().map(|(key, value)| (key.clone(), value.iter().map(|style| (style.property().to_string(), style.value().to_string())).collect::<Vec<(String, String)>>())).collect(), if !single_css { Some(filename) } else { None }) {
                        collected = true;
                    }
                }
    ...
    pub fn create_interface(&self, package_name: &str, color_interface_name: &str, typography_interface_name: &str, theme_interface_name: &str) -> String {
        ...
        if color_keys.is_empty() && typography_keys.is_empty() {
            ...
            format!(
                ...
                color_keys.into_iter().map(|key| format!("{}:null;", convert_interface_key(&format!("${key}")))).collect::<String>(),
                typography_interface_name,
                typography_keys.into_iter().map(|key| format!("{}:null;", convert_interface_key(&key))).collect::<String>(),
    ...
    fn create_style(&self, map: &BTreeMap<u8, HashSet<StyleSheetProperty>>) -> String {
        ...
        for (level, props) in map.iter() {
            let (mut global_props, rest): (Vec<_>, Vec<_>) = props.iter().partition(|prop| matches!(prop.selector, Some(StyleSelector::Global(_, _))));
            global_props.sort();
            let (mut medias, mut sorted_props): (Vec<_>, Vec<_>) = rest.into_iter().partition(|prop| matches!(prop.selector, Some(StyleSelector::Media { query: _, selector: _ })));
            sorted_props.sort();
            medias.sort();
            let medias = {
            ...
            let break_point = if *level == 0 { None } else { Some(self.theme.breakpoints.iter().enumerate().find(|(idx, _)| (*idx as u8) == *level).map(|(_, bp)| *bp).unwrap_or_else(|| self.theme.breakpoints.last().cloned().unwrap_or(0))) };
            ...
            if !global_props.is_empty() {
                ...
                for (selector, props) in selector_map {
                    inner_css.push_str(&format!("{}{{{}}}", selector, props.into_iter().map(|prop| format!("{}:{}", prop.property, prop.value)).collect::<Vec<String>>().join(";")));
                }
                current_css.push_str(if let Some(break_point) = break_point { format!("@media(min-width:{break_point}px){{{inner_css}}}") } else { inner_css }.as_str());
            }

            if !sorted_props.is_empty() {
                let inner_css = sorted_props.into_iter().map(ExtractStyle::extract).collect::<String>();
                current_css.push_str(if let Some(break_point) = break_point { format!("@media(min-width:{break_point}px){{{inner_css}}}") } else { inner_css }.as_str());
            }
            for (media, props) in medias {
                let inner_css = props.into_iter().map(ExtractStyle::extract).collect::<String>();
                current_css.push_str(if let Some(break_point) = break_point { format!("@media(min-width:{break_point}px)and {media}{{{inner_css}}}") } else { format!("@media{}{{{}}}", if media.starts_with("(") { media.clone() } else { format!(" {media}") }, inner_css.as_str()) }.as_str());
            }
        }
        current_css
    }
    ...
    fn create_header(&self) -> String {
        format!(
            ...
            version = include_str!("../../../bindings/devup-ui-wasm/package.json").lines().find(|line| line.contains("\"version\"")).unwrap().split(":").nth(1).unwrap().trim().replace("\"", ""),
        )
    }

    pub fn create_css(&self, filename: Option<&str>, import_main_css: bool) -> String {
        let header = self.create_header();
        let mut css = format!("{header}{}", self.imports.values().flatten().map(|import| { if import.starts_with("\"") { format!("@import {import};") } else { format!("@import \"{import}\";") } }).collect::<String>());
        ...
        if write_global {
            ...
            self.properties.values().for_each(|map| {
                ...
                if let Some(_base_styles) = map.get(&0) {
                    _base_styles.iter().for_each(|prop| {
                        base_styles.entry(*prop.0).or_default().extend(prop.1.clone());
                    });
                }
            });
            // default
            ...
            for (_, font_faces) in self.font_faces.iter() {
                for font_face in font_faces.iter() {
                    css.push_str(&format!("@font-face{{{}}}", font_face.iter().map(|(key, value)| format!("{key}:{value}")).collect::<Vec<String>>().join(";")));
                }
            }
        ...
        if let Some(keyframes) = self.keyframes.get(filename.unwrap_or_default()) {
            for (name, map) in keyframes {
                css.push_str(&format!("@keyframes {name}{{{}}}", map.iter().map(|(key, props)| format!("{key}{{{}}}", props.iter().map(|(key, value)| format!("{key}:{value}")).collect::<Vec<String>>().join(";"))).collect::<String>()));
            }
        }
        ...
        if let Some(maps) = self.properties.get(filename.unwrap_or_default()) {
            for (style_order, map) in maps.iter() {
                ...
                if !current_css.is_empty() {
                    ...
                    css.push_str(if *style_order == 255 { current_css } else { format!("@layer o{}{{{current_css}}}", style_order) }.as_str());
                }
            }
        }
        css
...
mod tests {
    ...
    fn test_convert_theme_variable_value() {
        ...
        assert_eq!(convert_theme_variable_value("$var $var"), "var(--var) var(--var)");

        assert_eq!(convert_theme_variable_value("1px solid $red"), "1px solid var(--red)");
    }
    ...
    fn test_create_css_with_selector_sort_test() {
        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background-color", 1, "red", Some(&"hover".into()), None, None);
        sheet.add_property("test", "background-color", 1, "some", None, None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());

        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background-color", 1, "red", None, None, None);
        sheet.add_property("test", "background-color", 1, "some", Some(&"hover".into()), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());
    ...
    fn test_import_css() {
        ...
        assert_debug_snapshot!(sheet.create_css(Some("index.tsx"), true).split("*/").nth(1).unwrap());
    }
    ...
    fn test_create_css() {
        ...
        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background", 0, "red", Some(&"hover".into()), None, None);
        sheet.add_property("test", "background", 0, "blue", Some(&"active".into()), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());

        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background", 0, "red", Some(&StyleSelector::from("group-focus-visible")), None, None);
        sheet.add_property("test", "background", 0, "blue", Some(&StyleSelector::from("group-focus-visible")), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());

        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background", 0, "red", Some(&StyleSelector::from("group-focus-visible")), None, None);
        sheet.add_property("test", "background", 0, "blue", Some(&StyleSelector::from("hover")), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());
        ...
        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background", 0, "red", Some(&["theme-dark", "hover"].into()), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());

        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background", 0, "red", Some(&["wrong", "hover"].into()), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());

        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background", 0, "red", Some(&"*[disabled='true'] &:hover".into()), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());

        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background", 0, "red", Some(&"&[disabled='true']".into()), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());

        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background", 0, "red", Some(&"&[disabled='true'], &[disabled='true']".into()), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());
    }
    ...
    fn test_selector_with_prefix() {
        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "margin-left", 1, "40px", Some(&"group-hover".into()), None, None);
        sheet.add_property("test", "margin-right", 1, "40px", Some(&"group-hover".into()), None, None);
        sheet.add_property("test", "margin-left", 2, "50px", Some(&"group-hover".into()), None, None);
        sheet.add_property("test", "margin-right", 2, "50px", Some(&"group-hover".into()), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());
    }
    ...
    fn test_theme_selector() {
        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "margin-left", 0, "40px", Some(&"theme-dark".into()), None, None);
        sheet.add_property("test", "margin-right", 0, "40px", Some(&"theme-dark".into()), None, None);
        sheet.add_property("test", "margin-top", 0, "40px", Some(&"theme-dark".into()), None, None);
        sheet.add_property("test", "margin-bottom", 0, "40px", Some(&"theme-dark".into()), None, None);
        sheet.add_property("test", "margin-left", 0, "50px", Some(&"theme-light".into()), None, None);
        sheet.add_property("test", "margin-right", 0, "50px", Some(&"theme-light".into()), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());
        ...
        sheet.add_property("test", "margin-left", 0, "51px", Some(&"theme-light".into()), None, None);
        sheet.add_property("test", "margin-right", 0, "51px", Some(&"theme-light".into()), None, None);
        sheet.add_property("test", "margin-left", 0, "42px", None, None, None);
        sheet.add_property("test", "margin-right", 0, "42px", None, None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());

        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "margin-left", 0, "50px", Some(&["theme-light", "active"].into()), None, None);
        sheet.add_property("test", "margin-right", 0, "50px", Some(&["theme-light", "active"].into()), None, None);
        sheet.add_property("test", "margin-left", 0, "50px", Some(&["theme-light", "hover"].into()), None, None);
        sheet.add_property("test", "margin-right", 0, "50px", Some(&["theme-light", "hover"].into()), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());
    }
    ...
    fn test_print_selector() {
        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "margin-left", 0, "40px", Some(&"print".into()), None, None);
        sheet.add_property("test", "margin-right", 0, "40px", Some(&"print".into()), None, None);
        sheet.add_property("test", "margin-top", 0, "40px", Some(&"print".into()), None, None);
        sheet.add_property("test", "margin-bottom", 0, "40px", Some(&"print".into()), None, None);

        sheet.add_property("test", "margin-left", 1, "40px", Some(&"print".into()), None, None);
        sheet.add_property("test", "margin-right", 1, "40px", Some(&"print".into()), None, None);
        sheet.add_property("test", "margin-top", 1, "40px", Some(&"print".into()), None, None);
        sheet.add_property("test", "margin-bottom", 1, "40px", Some(&"print".into()), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());
    ...
    fn test_selector_with_query() {
        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "margin-top", 0, "40px", Some(&StyleSelector::Media { query: "(min-width: 1024px)".to_string(), selector: Some("&:hover".to_string()) }), None, None);
        sheet.add_property("test", "margin-bottom", 0, "40px", Some(&StyleSelector::Media { query: "(min-width: 1024px)".to_string(), selector: Some("&:hover".to_string()) }), None, None);
    ...
    fn test_create_css_with_global_selector() {
        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background-color", 0, "red", Some(&StyleSelector::Global("div".to_string(), "test.tsx".to_string())), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());
        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background-color", 1, "red", Some(&StyleSelector::Global("div".to_string(), "test.tsx".to_string())), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());

        let mut sheet = StyleSheet::default();

        sheet.add_property("test", "background-color", 2, "blue", Some(&StyleSelector::Global("div".to_string(), "test.tsx".to_string())), None, None);
        sheet.add_property("test", "background-color", 1, "red", Some(&StyleSelector::Global("div".to_string(), "test.tsx".to_string())), None, None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());

        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background-color", 1, "blue", Some(&StyleSelector::Global("div".to_string(), "test.tsx".to_string())), Some(0), None);
        sheet.add_property("test", "background-color", 0, "red", Some(&StyleSelector::Global("div".to_string(), "test.tsx".to_string())), Some(255), None);
        assert_debug_snapshot!(sheet.create_css(None, false).split("*/").nth(1).unwrap());

        sheet.add_property("test", "background-color", 0, "red", Some(&StyleSelector::Global("div".to_string(), "test2.tsx".to_string())), Some(255), None);

        sheet.add_property("test2", "background-color", 0, "red", Some(&StyleSelector::Selector("&:hover".to_string())), Some(255), None);
        ...
        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background-color", 1, "blue", Some(&StyleSelector::Global("div".to_string(), "test.tsx".to_string())), Some(0), None);
        sheet.add_property("test", "color", 1, "blue", Some(&StyleSelector::Global("div".to_string(), "test.tsx".to_string())), Some(0), None);
        ...
        let mut sheet = StyleSheet::default();
        sheet.add_property("test", "background-color", 0, "blue", Some(&StyleSelector::Global("div".to_string(), "test.tsx".to_string())), Some(0), None);
        sheet.add_property("test", "color", 0, "blue", Some(&StyleSelector::Global("div".to_string(), "test2.tsx".to_string())), Some(0), None);
    ...
    fn test_get_theme_interface() {
        ...
        assert_eq!(sheet.create_interface("package", "ColorInterface", "TypographyInterface", "ThemeInterface"), "import \"package\";declare module \"package\"{interface ColorInterface{$primary:null;}interface TypographyInterface{}interface ThemeInterface{dark:null;}}");
        ...
        sheet.set_theme(theme);
        assert_eq!(sheet.create_interface("package", "ColorInterface", "TypographyInterface", "ThemeInterface"), "import \"package\";declare module \"package\"{interface ColorInterface{[`$(primary)`]:null;}interface TypographyInterface{[`prim\\`\\`ary`]:null;}interface ThemeInterface{dark:null;}}");
    }
    ...
    fn test_keyframes() {
        ...
        from_props.insert(StyleSheetProperty { class_name: String::from("test"), property: String::from("opacity"), value: String::from("0"), selector: None });
        keyframes.insert(String::from("from"), vec![(String::from("opacity"), String::from("0"))]);

        let mut to_props = BTreeSet::new();
        to_props.insert(StyleSheetProperty { class_name: String::from("test"), property: String::from("opacity"), value: String::from("1"), selector: None });
        keyframes.insert(String::from("to"), vec![(String::from("opacity"), String::from("1"))]);
    ...
    fn test_font_face() {
        ...
        font_face_props.insert("src".to_string(), "url('/fonts/Roboto-Regular.ttf')".to_string());
