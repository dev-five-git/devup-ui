use crate::{
    ...
    extractor::{ExtractResult, KeyframesExtractResult, extract_style_from_expression::extract_style_from_expression},
...
pub fn extract_keyframes_from_expression<'a>(ast_builder: &AstBuilder<'a>, expression: &mut Expression<'a>) -> KeyframesExtractResult {
    ...
    if let Expression::ObjectExpression(obj) = expression {
        for p in obj.properties.iter_mut() {
            ...
            {
                let ExtractResult { styles, .. } = extract_style_from_expression(ast_builder, None, &mut o.value, 0, &None);
                ...
                keyframes.keyframes.insert(name.parse::<f64>().map(|v| format!("{v}%")).unwrap_or(name), styles);
            }
        }
    }
